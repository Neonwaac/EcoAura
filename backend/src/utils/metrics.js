...
const productMetricsQuery = `
  SELECT
    ...
    CASE
      WHEN COALESCE(tp.total_purchased, 0) > 0 THEN COALESCE(ts.full_paid_revenue, 0) - (COALESCE(ts.full_paid_units, 0) * (COALESCE(tp.total_purchase_cost_no_shipping, 0) / tp.total_purchased)) + COALESCE(ts.partial_collected, 0)
        ELSE 0
    END AS profit
  FROM products p
  LEFT JOIN (
  ...
  LEFT JOIN (
    SELECT
      ...
      SUM(CASE WHEN payment_status = 'Pagado' THEN quantity * unit_price ELSE 0 END) AS full_paid_revenue,
